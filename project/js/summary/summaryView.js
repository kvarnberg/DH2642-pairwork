function SummaryView({
  ...
  return h(
    ...
    h(
      "button",
      { onClick: doneCallback, label: doneMessage, style: "float:right" },
      "Back to search"
    ),
    ...
    h(
      ...
      h(
        ...
        h(
          "td",
          model.getMenu().map((dish) => displayMenu(dish))
        ),
        h(
          "td",
          ingredients.map((ing) => displayShopping(ing))
        ),
        h(
          "td",
          ingredients.map((ing) => h("li", ing.aisle))
        ),
